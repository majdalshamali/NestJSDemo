...
async function bootstrap() {
  ...
  // Swagger only covers Nest's own controllers (/, /api/*, /users). Better
  // Auth's routes are generated dynamically and never go through Nest's
  // router, so @nestjs/swagger cannot see them; they get their own
  // reference page instead, via the openAPI() plugin in auth/auth.ts
  ...
  const swaggerConfig = new DocumentBuilder()
    ...
    .setDescription(
      'Auth (sign up, sign in, session, admin) is generated by Better Auth ' +
        ...
        'every route Nest itself serves: /users, the Arcjet demos under ' +
        '/api, and the root route.',
    )
    .setVersion('1.0')
    .addBearerAuth({
      type: 'http',
